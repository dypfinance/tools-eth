...
async function getOldTokenResponse(tokenIdList, _24hEarlierBlock) {
  ...
  let oldTokenResponses = await Promise.all(tokenIdList.map(token => fetchGql(TOKEN_HISTORY_QUERY, {token, block: _24hEarlierBlock}, 'https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v2')))
...
export default async function getTopTokens() {
    ...
    let response = await fetchGql(TOKENS_QUERY, null, 'https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v2')
