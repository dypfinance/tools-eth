...
async function getPairCandles(pair, from, to, n=0) {
    ...
    let candles = await axios.get(`${window.config.api_baseurl}/api/candles/minutes/${pair}${query}`)
...
export default function getDataFeed({symbol, pairId, registerBarSubscription, onBarsRequest, mainToken}) {
    ...
    return ({
        ...
        getBars: async (symbolInfo, resolution, from, to, onHistoryCallback, onErrorCallback, firstDataRequest) => {
            ...
            let pairCandles
            if (firstDataRequest) {
                pairCandles = await getPairCandles(pairId, undefined, undefined, mainToken.__number)
            } else {
                pairCandles = await getPairCandles(pairId, from, to, mainToken.__number)
            }
