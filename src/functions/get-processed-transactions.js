export default function getProcessedTransactions() {
    return new Promise((resolve, reject) => {
        fetch(window.config.subgraph_url || 'https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v2', {
            method: 'POST',
            mode: 'cors',
...
function handleTheGraphData({data}) {
    ...
    let burns = data.burns.filter(mintOrBurn => {
        return [mintOrBurn.pair.token0.id, mintOrBurn.pair.token1.id].includes(window.config.weth_address)
...
function getFormattedMintOrBurn(mintOrBurn, type) {
    ...
    if (token0.id == window.config.weth_address) {
